...
from agents.tool_codegen import (
    build_tool_body,
    get_module_helpers,
...
def build_tool_implementation(tool: ToolSpec, req: MCPRequirement) -> ToolImplementation:
    ...
    if not sig and tool.parameters:
        sig = _params_to_signature(tool.parameters)
...
def build_resource_implementation(resource) -> ResourceImplementation:
    params_sig = _params_to_signature(resource.params) if resource.params else ""
...
def get_module_helpers(req: MCPRequirement) -> str:
    from agents.tool_codegen import get_module_helpers as _helpers
