...
from agents.codegen_core import (
...
def signature_for_tool(tool: ToolSpec, domain: str) -> str:
    if domain == "code_generator":
        if tool.parameters and _has_specification_param(tool):
            return _params_to_signature(tool.parameters)
...
def _params_to_signature(params: list[ParameterSpec]) -> str:
    parts: list[str] = []
    for p in params:
        ptype = p.type or "str"
        if p.required:
...
def build_tool_body(tool: ToolSpec, req: MCPRequirement, domain: str | None = None) -> str:
    ...
    if domain == "microservice_layer":
        from agents.factory_codegen import factory_tool_body
...
def get_module_helpers(req: MCPRequirement) -> str:
    ...
    from agents.factory_codegen import FACTORY_MODULE_HELPERS, is_microservice_factory_requirement
