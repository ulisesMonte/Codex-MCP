"""Infer technical MCP details from natural-language user input."""
...
from agents.requirement_intent import apply_intent_to_requirement
...
def combined_user_text(messages: list[BaseMessage]) -> str:
    parts = [str(m.content).strip() for m in messages if isinstance(m, HumanMessage) and m.content]
    return "\n".join(parts)


def user_confirmed(text: str) -> bool:
    """Final go-ahead to generate the MCP."""
    t = text.strip()
    if _CONFIRM_RE.match(t):
        return True
    if _CONFIRM_TYPO_RE.match(t):
        return True
    normalized = re.sub(r"[^a-z]", "", t.lower())
    if normalized and difflib.get_close_matches(
...
def user_accepts_proposal(text: str) -> bool:
    """User agrees with the agent's proposal (not only bare 'confirm')."""
    t = text.strip()
    if not t:
        return False
    if user_rejects_proposal(t):
        return False
    if user_confirmed(t) or is_affirmative(t):
        return True
    return bool(_CONFIRMATION_INTENT_RE.search(t))


def user_rejects_proposal(text: str) -> bool:
    """User explicitly rejected the agent's proposal or asked for changes."""
    t = text.strip()
    if not t:
        return False
    if user_confirmed(t):
    ...
    if _USER_REJECT_RE.search(t):
        return True
    if re.search(r"\bno\b", t, re.I) and re.search(
...
def agent_asked_confirmation(messages: list[BaseMessage]) -> bool:
    agent = last_agent_message(messages)
    return bool(_AGENT_ASKED_CONFIRM_RE.search(agent))


def user_sent_tool_correction(text: str) -> bool:
    """User is correcting tool count/names (not just confirming)."""
    if not text.strip():
        return False
    if _USER_TOOL_CORRECTION_RE.search(text):
        return True
    factories = _FACTORY_NAME_RE.findall(text)
    return len(factories) >= 2


def last_agent_message(messages: list[BaseMessage]) -> str:
...
def enrich_requirement(req: MCPRequirement, user_text: str, messages: list[BaseMessage] | None = None) -> MCPRequirement:
    """Fill agent-side fields the user should not have to specify."""
    text = user_text or ""
    msgs = messages or []
    ...
    apply_intent_to_requirement(req, combined_user_text(msgs) if msgs else text)

    return req


def finalize_requirement(req: MCPRequirement, messages: list[BaseMessage]) -> MCPRequirement:
    """Last pass before confirm — requirement must match user intent."""
    ...
    _infer_mcp_name(req, text)
    apply_intent_to_requirement(req, text)
    return req
...
def _ensure_tool_parameters(req: MCPRequirement) -> None:
    ...
    for tool in req.tools:
        ...
        if tool.name in _CRUD_OPS:
            from agents.requirement_intent import resolved_intent, GenerationIntent
...
def build_confirmation_message(req: MCPRequirement) -> str:
    ...
    lines.append("")
    lines.append("¿Es correcto? Escribí **confirm** para generarlo o contame qué cambiar.")
    return "\n".join(lines)


def user_facing_gaps(req: MCPRequirement) -> list[str]:
    """Console hints only (English). Agent chat may still reply in the user's language."""
    if req.is_ready():
    ...
    for note in req.clarifications_needed:
        text = note.strip()
        if text and not _is_agent_side_clarification(text):
            gaps.append(text)
...
def decide_status(
    ...
    if user_rejects_proposal(last_user_message):
        return "gathering"
    if not req.is_ready():
        return "gathering"
    if user_confirmed(last_user_message):
        return "complete"
    msgs = messages or []
    if msgs and agent_asked_confirmation(msgs) and user_accepts_proposal(last_user_message):
        return "complete"
    return "gathering"  # waiting for explicit confirm
